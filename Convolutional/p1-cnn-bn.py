...
def train_and_evaluate(batch_size=64, learning_rate=0.001, num_epochs=15, force_retrain=False, use_early_stopping=True):
    ...
    model_save_path = f'cnn_bn_model_batch{batch_size}_lr{learning_rate}.pth'
    ...
    if os.path.exists(model_save_path) and not force_retrain:
        ...
        # Training loop
        print(f"Starting training with batch size: {batch_size}, learning rate: {learning_rate}")
        ...
        plt.tight_layout()
        plt.savefig(f'cnn_bn_batch{batch_size}_lr{learning_rate}_curves.png')
...
def main():
    parser = argparse.ArgumentParser(description='Train and evaluate ConvNet with batch normalization using different batch sizes')
...
# Function to run a series of batch size experiments for analyzing batch normalization
def run_batch_size_experiments():
    # Batch sizes to test
    batch_sizes = [2, 16, 64, 256]
    ...
    print(f"{'Batch Size':<15} {'Accuracy':>10}")
    print(f"{'-'*15} {'-'*10}")
    for result in results:
        print(f"{result['batch_size']:<15} {result['accuracy']:>10.2f}%")
    
    # Plot batch size vs accuracy
    plt.figure(figsize=(10, 6))
    plt.plot([r['batch_size'] for r in results], [r['accuracy'] for r in results], 'bo-')
    plt.xscale('log', base=2)  # Log scale for batch size
    plt.title('Batch Size Effect on Accuracy with Batch Normalization')
    plt.xlabel('Batch Size (log scale)')
    plt.ylabel('Test Accuracy (%)')
    plt.grid(True)
    plt.savefig('batch_size_impact.png')
    print("Batch size impact plot saved as 'batch_size_impact.png'")

if __name__ == "__main__":
    # Uncomment to run batch size experiments
    run_batch_size_experiments()
