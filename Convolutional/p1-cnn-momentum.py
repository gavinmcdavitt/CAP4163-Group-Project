...
def train_and_evaluate(momentum=0.9, learning_rate=0.01, num_epochs=15, batch_size=64, force_retrain=False, use_early_stopping=True):
    ...
    # Use SGD with momentum for this experiment
    optimizer = optim.SGD(model.parameters(), lr=learning_rate, momentum=momentum)
    
    # Define model save path based on parameters
    model_save_path = f'cnn_momentum{momentum}_lr{learning_rate}.pth'
    ...
    if os.path.exists(model_save_path) and not force_retrain:
        ...
        # Training loop
        print(f"Starting training with momentum: {momentum}, learning rate: {learning_rate}")
        for epoch in range(num_epochs):
            ...
            print(f'Epoch [{epoch+1}/{num_epochs}], Time: {epoch_time:.2f}s, Loss: {avg_loss:.4f}, '
        ...
        plt.title(f'Training Loss (Momentum={momentum})')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.grid(True)
        ...
        plt.title(f'Training Accuracy (Momentum={momentum})')
        plt.xlabel('Epoch')
        plt.ylabel('Accuracy (%)')
        plt.grid(True)
        ...
        plt.title(f'Test Accuracy (Momentum={momentum})')
        plt.xlabel('Epoch')
        plt.ylabel('Accuracy (%)')
        plt.grid(True)
        ...
        plt.title(f'Epoch Training Time (Momentum={momentum})')
        plt.xlabel('Epoch')
        plt.ylabel('Time (s)')
        plt.grid(True)
        
        plt.tight_layout()
        plt.savefig(f'cnn_momentum{momentum}_lr{learning_rate}_curves.png')
...
def main():
    parser = argparse.ArgumentParser(description='Train and evaluate ConvNet with different momentum values')
...
# Function to run a series of momentum experiments
def run_momentum_experiments():
    ...
    for momentum in momentum_values:
        ...
        accuracy, _ = train_and_evaluate(
            momentum=momentum,
            learning_rate=0.01,  # Fixed learning rate for all experiments
    ...
    print("\n\nSummary of Momentum Experiments:")
    print(f"{'Momentum':<15} {'Accuracy':>10}")
    print(f"{'-'*15} {'-'*10}")
    for result in results:
        print(f"{result['momentum']:<15} {result['accuracy']:>10.2f}%")
    
    # Plot momentum vs accuracy
    plt.figure(figsize=(10, 6))
    plt.plot([r['momentum'] for r in results], [r['accuracy'] for r in results], 'bo-')
    plt.title('Momentum Effect on Accuracy')
    plt.xlabel('Momentum Value')
    plt.ylabel('Test Accuracy (%)')
    plt.grid(True)
    plt.savefig('momentum_impact.png')
    print("Momentum impact plot saved as 'momentum_impact.png'")

if __name__ == "__main__":
    # Uncomment to run momentum experiments
